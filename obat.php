      ...
      <div class="card-body">
        <table class="table table-responsive-md table-striped table-hover">
          ...
          <tbody>
            <?php
            $query = "SELECT
                          bu.*, pt.nama as jenis, ps.nama as satuan
                      FROM
                          obat AS bu
                      Left JOIN jenis AS pt
                      ON
                          bu.jenis_id = pt.id
                      Left JOIN satuan AS ps
                      ON
                          bu.satuan_id = ps.id
            ";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              $btn_edit =  '<a href="./obat-form.php?edit=' . $row->id . '" class="btn btn-sm btn-primary">Edit</a>';
              $btn_delete =  '<a href="./obat.php?delete=' . $row->id . '" onclick="return confirm(\'Apakah anda yakin?\')" class="btn btn-sm btn-danger">Hapus</a>';
              echo "<tr>
              <th>$counter</th>
              <td>{$row->nama}</td>
              <td>{$row->satuan}</td>
